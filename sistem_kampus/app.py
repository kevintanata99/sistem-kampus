...
from werkzeug.utils import secure_filename # <--- TAMBAHAN 2
...
def get_db_connection():
    # Cek: Apakah ada alamat database dari Cloud (Railway)?
    db_url = os.environ.get('DATABASE_URL')
    
    if db_url:
        # KASUS 1: Sedang di Cloud (Pakai URL Railway)
        conn = psycopg2.connect(db_url)
    else:
        # KASUS 2: Sedang di Laptop (Pakai Localhost)
        conn = psycopg2.connect(
            host="localhost",
            database="postgres",
            user="postgres",
            password="12345", # Passwordmu aman di sini kalau cuma buat local
            port="5432"
        )
    return conn

# --- 2. RUTE LOGIN (OTAK UTAMA) ---
...
# --- 4. RUTE SEMENTARA (Biar gak error kalau redirect) ---
